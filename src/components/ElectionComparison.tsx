...
export const ElectionComparison: React.FC<ElectionComparisonProps> = ({ onSelectWard }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900/60 backdrop-blur-md border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm dark:shadow-xl space-y-4 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
          <div>
            <div className="flex items-center gap-2">
              <GitCompare className="w-6 h-6 text-emerald-500 dark:text-emerald-400" />
          ...
          {/* Selectors */}
          <div className="flex items-center gap-3 bg-slate-100 dark:bg-slate-800/40 p-2 rounded-full border border-slate-200 dark:border-slate-700/70">
            <div className="flex items-center gap-2">
              <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 pl-2">A:</span>
              <select
                value={yearA}
                onChange={(e) => setYearA(Number(e.target.value))}
                className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white text-xs font-bold rounded-full px-3 py-1.5 focus:outline-none focus:border-emerald-500 font-mono shadow-xs"
              >
            ...
            <span className="text-emerald-600 dark:text-emerald-500 font-mono font-bold text-xs">VS</span>

            <div className="flex items-center gap-2">
              <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">B:</span>
              <select
                value={yearB}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-2 relative z-10">
          {/* Turnout Shift */}
          <div className="bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700/60 p-4 rounded-2xl space-y-1">
            <div className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase font-mono">Turnout Shift (A vs B)</div>
            <div className="flex items-center gap-2">
              <span className="text-xl font-black text-slate-900 dark:text-white font-mono">{electionA.overallTurnout.toFixed(1)}% vs {electionB.overallTurnout.toFixed(1)}%</span>
            </div>
            <div className={`text-xs font-bold font-mono flex items-center gap-1 ${
              Number(turnoutDiff) >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400'
            }`}>
          ...
          <div className="bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700/60 p-4 rounded-2xl space-y-1">
            <div className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase font-mono">Mayoral Winners</div>
          ...
          {/* Ballots Difference */}
          <div className="bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700/60 p-4 rounded-2xl space-y-1">
            <div className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase font-mono">Total Ballots Cast</div>
          ...
          {/* Council Turnover */}
          <div className="bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700/60 p-4 rounded-2xl space-y-1">
            <div className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase font-mono">Council Turnover</div>
                  ...
                  <tr key={geo.wardNumber} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                    <td className="py-3 px-3 font-semibold text-slate-900 dark:text-white">
                      <button
                        ...
                        className="hover:text-emerald-600 dark:hover:text-emerald-400 hover:underline flex items-center gap-1.5 cursor-pointer"
                      >
                    ...
                    <td className="py-3 px-3">
                      <div className="font-bold text-slate-900 dark:text-slate-100">{wardA?.winner.name}</div>
                      <div className="text-[10px] text-emerald-600 dark:text-emerald-400 font-mono">
                        {wardA?.winner.votePercentage.toFixed(1)}% ({wardA?.winner.votes.toLocaleString()} votes)
                      </div>
                    </td>
                    <td className="py-3 px-3 text-center font-bold text-emerald-600 dark:text-emerald-400 font-mono">
                      {wardA?.turnoutPercentage.toFixed(1)}%
                    </td>

                    {/* Year B */}
                    <td className="py-3 px-3">
                      <div className="font-bold text-slate-900 dark:text-slate-100">{wardB?.winner.name}</div>
                      <div className="text-[10px] text-slate-500 dark:text-slate-400 font-mono">
                        {wardB?.winner.votePercentage.toFixed(1)}% ({wardB?.winner.votes.toLocaleString()} votes)
                      </div>
                    </td>
                    <td className="py-3 px-3 text-center font-bold text-slate-600 dark:text-slate-400 font-mono">
                    ...
                    <td className="py-3 px-3 text-center">
                      {sameWinner ? (
                        <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 text-[10px] font-bold border border-emerald-500/20 font-mono">
                          Retained Rep
                        </span>
                      ) : (
                        <span className="px-2.5 py-0.5 rounded-full bg-purple-500/20 text-purple-700 dark:text-purple-300 text-[10px] font-bold border border-purple-500/30 font-mono">
                          Flipped Rep
                        </span>
                      )}
                    </td>
