...
export const WardRacesGrid: React.FC<WardRacesGridProps> = ({
  ...
  return (
    <div className="space-y-6">
      {/* Ward Races Header */}
      <div className="bg-white dark:bg-slate-900/60 backdrop-blur-md border border-slate-200 dark:border-slate-800 rounded-3xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-sm dark:shadow-xl">
        <div>
        ...
        <div className="px-3 py-1 bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-500/20 rounded-full text-xs font-mono font-bold self-start sm:self-auto">
          {wardCount} Wards
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {election.wards.map((ward) => {
          ...
          return (
            <div
              key={ward.wardNumber}
              className={`bg-white dark:bg-slate-900/60 backdrop-blur-md border rounded-3xl p-5 space-y-3.5 hover:border-slate-300 dark:hover:border-slate-700 transition-all flex flex-col justify-between shadow-sm dark:shadow-lg ${
                isClose ? 'border-amber-500/40' : 'border-slate-200 dark:border-slate-800'
              }`}
            >
              <div>
                {/* Ward Top Info */}
                <div className="flex items-start justify-between gap-2 border-b border-slate-200 dark:border-slate-800 pb-3">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold px-2.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-500/20 font-mono">
                        Ward {ward.wardNumber}
                      </span>
                      {isClose && (
                        <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-700 dark:text-amber-300 border border-amber-500/30 flex items-center gap-1">
                          <Flame className="w-3 h-3 text-amber-500 dark:text-amber-400" />
                          Close Race
                        </span>
                      )}
                      {!ward.isIncumbentRetained && (
                        <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-purple-500/20 text-purple-700 dark:text-purple-300 border border-purple-500/30">
                          Flipped
                        </span>
                      )}
                    </div>
                ...
                {ward.isDualMemberWard && ward.councillors ? (
                  <div className="mt-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/60 rounded-2xl p-3 space-y-2">
                    <div className="flex items-center justify-between text-[11px] text-slate-500 dark:text-slate-400">
                      <span className="font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider flex items-center gap-1.5">
                        <Award className="w-3.5 h-3.5 text-emerald-500 dark:text-emerald-400" /> Elected Councillors (2 Seats)
                      </span>
                      <span className="text-[10px] text-slate-500 dark:text-slate-400 font-mono">Dual-Seat System</span>
                    </div>

                    <div className="space-y-1.5">
                      {ward.councillors.map((c, cIdx) => (
                        <div key={cIdx} className="bg-white dark:bg-slate-900/60 p-2 rounded-xl border border-slate-200 dark:border-slate-700/40 flex items-center justify-between text-xs">
                          <div className="flex items-center gap-1.5 font-bold text-slate-900 dark:text-white">
                            <ShieldCheck className="w-3.5 h-3.5 text-emerald-500 dark:text-emerald-400" />
                            <span>{c.name}</span>
                            <span className="text-[10px] text-emerald-700 dark:text-emerald-400/80 font-normal font-mono">({c.notes || `Seat ${cIdx === 0 ? 'A' : 'B'}`})</span>
                          </div>
                          <div className="font-mono text-emerald-600 dark:text-emerald-400 font-bold">
                            {c.votePercentage.toFixed(1)}% <span className="text-[10px] text-slate-500 font-normal">({c.votes})</span>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="mt-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/60 rounded-2xl p-3.5 space-y-2">
                    <div className="flex items-center justify-between text-[11px] text-slate-500 dark:text-slate-400">
                      <span className="font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider flex items-center gap-1.5">
                        <Award className="w-3.5 h-3.5 text-emerald-500 dark:text-emerald-400" /> Elected Councillor
                      </span>
                    ...
                    {ward.candidates.map((cand) => (
                      <div
                        ...
                        className={`py-1 px-2.5 rounded-xl text-xs flex items-center justify-between transition-colors ${
                          cand.isWinner
                            ? 'bg-emerald-500/10 text-emerald-800 dark:text-emerald-300 font-semibold border border-emerald-500/20'
                            : 'bg-slate-50 dark:bg-slate-800/30 text-slate-700 dark:text-slate-300'
                        }`}
                      >
                        <span className="truncate">{cand.name}</span>
                        <span className="font-bold shrink-0 ml-2 font-mono text-right text-slate-900 dark:text-white">
              ...
              <button
                ...
                className="w-full py-2.5 px-3 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800/80 dark:hover:bg-slate-700 text-slate-700 hover:text-slate-950 dark:text-slate-200 dark:hover:text-white rounded-2xl text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors border border-slate-200 dark:border-slate-700/60 mt-2 cursor-pointer"
              >
